...
function draw_map_text(zoom){
	...
	if zoom >= 15 {
		...
		for (i = 0; i < instance_number(obj_province); i ++) {
			...
			with instance_find(obj_province, i) {
				draw_set_halign(fa_center)
				draw_set_font(fnt_header)
				draw_text(x, y, global.provinces[prov_id][0])
		...
		for (i = 0; i < array_length(global.tags); i ++) {
			...
			if upmost_x > downmost_x {
				...
				angle = (arctan((downmost_y - upmost_y) / (leftmost_x - upmost_x))) * -(45/pi)
			} else { 
				// Top Left --> To --> Bottom Right
				angle = (arctan((downmost_y - upmost_y) / (rightmost_x - downmost_x))) * -(45/pi)
			}
			
			show_debug_message(global.tags[i][0] + ": " + string(angle))
			
			draw_set_halign(fa_center)
			draw_text_transformed(mid_x, mid_y, global.tags[i][1], 1, 1, angle)
			draw_set_halign(fa_left)
